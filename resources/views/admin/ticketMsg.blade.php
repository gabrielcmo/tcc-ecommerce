...
@section('content')
    ...
    <br>
    <div class="row">
    <form action="{{ route('admin.ticket.update', ['id'=>$ticket->id]) }}" method="post" class="w-100">
        <p class="font-weight-bolder">Assunto: <span class="font-weight-light">{{$ticket->subject}}</span></p>
        <p class="font-weight-bolder">Tipo da dúvida: <span class="font-weight-light">{{$ticket_type->name}}</span></p>
